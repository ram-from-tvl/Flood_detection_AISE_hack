# ============================================================
# ITER 5 — CELL 2: Train + Predict
# Requires Cell 1 to have been run first.
# ============================================================

# ---------------------------------------------------------------------------
# Datamodule — proper train/val split (not trainval)
# Using train.txt for training, val.txt for monitoring
# This lets us actually see overfitting vs iter4 which used trainval for both
# ---------------------------------------------------------------------------
train_aug = [
    ...
    A.Transpose(p=0.3),
    # Radiometric augmentation — simulate SAR speckle variation
    A.GaussNoise(var_limit=(0.01, 0.05), p=0.4),
    A.RandomBrightnessContrast(brightness_limit=0.15, contrast_limit=0.15, p=0.4),
    # Spatial augmentation — simulate different patch positions
...
# ---------------------------------------------------------------------------
# Model — warm-start from iter4 checkpoint
# ---------------------------------------------------------------------------
...
model = FloodSegmentationTask(
    model_args = dict(
        ...
        backbone_pretrained   = False,   # weights come from iter4 ckpt
        ...
        num_classes  = CFG["NUM_CLASSES"],
        head_dropout = CFG["HEAD_DROPOUT"],   # 0.3 vs iter4's 0.1
        rescale      = True,
    ),
...
# Load iter4 weights
iter4_ckpt = CFG["ITER4_CKPT"]
if iter4_ckpt and os.path.exists(iter4_ckpt):
    state = torch.load(iter4_ckpt, map_location="cpu")
    missing, unexpected = model.load_state_dict(state["state_dict"], strict=False)
    sys.__stdout__.write(f"Warm-start from iter4: missing={len(missing)} unexpected={len(unexpected)}\n")
else:
    sys.__stdout__.write(f"iter4 ckpt not found — cold start with pretrained backbone\n")
    # Fall back to pretrained backbone
    model.model_args = dict(model.model_args, backbone_pretrained=True)
...
trainer = pl.Trainer(
    ...
    callbacks               = [
        ckpt,
        EarlyStopping(monitor="val/IoU_Flood", mode="max", patience=CFG["ES_PATIENCE"]),
...
# ---------------------------------------------------------------------------
sys.__stdout__.write(
    f"\nTraining iter5: {CFG['EPOCHS']} epochs | "
    f"lr_decoder={CFG['LR_DECODER']} | lr_backbone={CFG['LR_BACKBONE']}\n"
    f"label_smooth={CFG['LABEL_SMOOTH']} | head_dropout={CFG['HEAD_DROPOUT']}\n"
)
t0 = datetime.now()
trainer.fit(model, datamodule=datamodule)
duration  = datetime.now() - t0
best_ckpt = ckpt.best_model_path or ckpt.last_model_path
...
for tid in submit_ids:
    ...
    if pred_arr is None:
        sys.__stdout__.write(f"  WARNING: missing {tid}\n")
...
sub_csv = os.path.join(CFG["OUT_DIR"], "submission_iter5.csv")
...
summary = {
    ...
    "best_ckpt"          : best_ckpt,
    "warm_start_from"    : CFG["ITER4_CKPT"],
    "config"             : CFG,
    "issues"             : issues,
}
json_path = os.path.join(CFG["OUT_DIR"], "iter5_summary.json")
...
with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
    ...
    zf.write(json_path, "iter5_summary.json")
    if os.path.exists(metrics_csv):
...
sys.__stdout__.write("Submit submission_iter5.csv\n")
