# ============================================================
# CELL 2 — Re-run this when fixing bugs / changing hyperparams
# Datamodule → model → train → predict → submit
# Requires Cell 1 to have been run first in the same session.
# ============================================================
...
datamodule = GenericNonGeoSegmentationDataModule(
    ...
    means          = MEANS,
    stds           = STDS,
    train_transform= [A.HorizontalFlip(p=0.5), A.VerticalFlip(p=0.5),
...
model = FloodSegmentationTask(
    ...
    ignore_index      = -1,
    freeze_backbone   = True,   # no grads through 300M ViT → fits in 15GB at fp32
...
# Trainer
# ---------------------------------------------------------------------------
# Allow TF32 on T4 — fixes cuBLAS alignment errors with fp16/mixed precision
...
# Train
# ---------------------------------------------------------------------------
print(f"\nTraining: {CFG['EPOCHS']} epochs | batch={CFG['BATCH_SIZE']} | lr={CFG['LR']}")
t0 = datetime.now()
...
# Fallback: ordered inference if batch had no filename metadata
if len(all_preds) == 0:
...
sub_csv = os.path.join(CFG["OUT_DIR"], "submission_iter1.csv")
...
json_path = os.path.join(CFG["OUT_DIR"], "iter1_summary.json")
with open(json_path, "w") as f:
    json.dump(summary, f, indent=2, default=str)

zip_path = "/kaggle/working/submission_iter1.zip"
with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
    zf.write(sub_csv,   "submission_iter1.csv")
    zf.write(json_path, "iter1_summary.json")
    if os.path.exists(metrics_csv):
...
sys.__stdout__.write("Submit submission_iter1.csv\n")
