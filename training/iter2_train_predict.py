# ============================================================
# ITER 2 — CELL 2: Re-run when tweaking
# Full backbone finetune, warm-started from iter1 checkpoint
# ============================================================

# ---------------------------------------------------------------------------
# Datamodule — same as iter1, stronger augmentation added
# ---------------------------------------------------------------------------
datamodule = GenericNonGeoSegmentationDataModule(
    ...
    test_data_root        = CFG["IMG_DIR"],
    test_label_data_root  = CFG["LABEL_DIR"],
    predict_data_root     = CFG["PRED_DIR"],
    train_split    = os.path.join(CFG["SPLIT_DIR"], "train.txt"),
    val_split      = os.path.join(CFG["SPLIT_DIR"], "val.txt"),
    test_split     = os.path.join(CFG["SPLIT_DIR"], "test.txt"),
...
# ---------------------------------------------------------------------------
# Model — unfreeze backbone, warm-start from iter1 checkpoint
# ---------------------------------------------------------------------------
...
# Warm-start: load iter1 decoder/head weights, skip backbone (already pretrained)
iter1_ckpt = CFG["ITER1_CKPT"]
if os.path.exists(iter1_ckpt):
    state = torch.load(iter1_ckpt, map_location="cpu")
    missing, unexpected = model.load_state_dict(state["state_dict"], strict=False)
    sys.__stdout__.write(f"Warm-start from iter1 ckpt\n")
    sys.__stdout__.write(f"  missing={len(missing)}  unexpected={len(unexpected)}\n")
else:
    sys.__stdout__.write(f"iter1 ckpt not found at {iter1_ckpt} — starting from scratch\n")

# Gradient checkpointing — trades compute for memory, enables full finetune on T4
...
sys.__stdout__.write(f"Model: {total/1e6:.0f}M total | {trainable/1e6:.0f}M trainable\n")
...
trainer = pl.Trainer(
    ...
    devices                 = 1,
    precision               = "32",
    accumulate_grad_batches = 2,        # effective batch = 4
    max_epochs              = CFG["EPOCHS"],
    logger                  = CSVLogger(CFG["OUT_DIR"], name="logs"),
    callbacks               = [
        ckpt,
        EarlyStopping(monitor="val/mIoU", mode="max", patience=CFG["ES_PATIENCE"]),
...
print(f"\nTraining: {CFG['EPOCHS']} epochs | batch={CFG['BATCH_SIZE']} | "
      f"lr_decoder={CFG['LR']} | lr_backbone={CFG['LR_BACKBONE']}")
t0 = datetime.now()
...
if best_ckpt and os.path.exists(best_ckpt):
    ...
    sys.__stdout__.write(f"  Loaded weights from {os.path.basename(best_ckpt)}\n")
...
sys.__stdout__.write(f"  Predictions in memory: {len(all_preds)} patches\n")

if len(all_preds) == 0:
    sys.__stdout__.write("  No filename metadata — fallback ordered inference\n")
    with torch.no_grad():
        for pid in sorted(pred_file_map.keys()):
            ...
            all_preds[pid] = pred
    sys.__stdout__.write(f"  Fallback done: {len(all_preds)} patches\n")
...
for tid in submit_ids:
    ...
    if pred_arr is None:
        ...
        if fp:
            ...
            sys.__stdout__.write(f"  WARNING: no prediction for {tid} — using '0 0'\n")
...
sub_csv = os.path.join(CFG["OUT_DIR"], "submission_iter2.csv")
...
json_path = os.path.join(CFG["OUT_DIR"], "iter2_summary.json")
with open(json_path, "w") as f:
    json.dump(summary, f, indent=2, default=str)

zip_path = "/kaggle/working/submission_iter2.zip"
with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
    zf.write(sub_csv,   "submission_iter2.csv")
    zf.write(json_path, "iter2_summary.json")
    if os.path.exists(metrics_csv):
        zf.write(metrics_csv, "training_metrics.csv")

sys.__stdout__.write(f"\nZIP: {zip_path} ({os.path.getsize(zip_path)/1024:.1f} KB)\n")
sys.__stdout__.write(f"val/IoU_Flood best = {summary['best_val_IoU_Flood']}\n")
sys.__stdout__.write("Submit submission_iter2.csv\n")
